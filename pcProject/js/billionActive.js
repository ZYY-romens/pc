...
//				累计投资不在10位或者11位时,显示*
			...
			for(var i=0;i<11;i++){
				imgs="<img src='../img/digital-"+parseInt(i)+".png' />";
...
//    		百亿活动结束
    		if(isEnd == 1){
    			...
    			$('#stage_dom'+crownId).children('img').attr("src","../img/crown.png");
    			$('#finish_'+crownId,'#finish_'+(crownId+1)).addClass('win-section');
    			$('#finish_'+crownId,'#finish_'+(crownId+1)).children('img').attr("src","../img/diamond-03.png");
...
//    		奖励设置发放切换点隐藏,默认显示奖励设置
    			...
    			$('.reward-countining').each(function(){
    				if($(this).hasClass('now-section')){
    					 $(this).children('img').attr("src","../img/diamond-03.png");
    				}else{
    					 $(this).children('img').attr("src","../img/diamond-01.png");
    				}
    			})
    	    }
...
function CountDown(){
	...
	var billionThirdStarFlag = $("#billionThirdStarFlag").val();
	if(billionThirdStarFlag!=1){
		return;
	}
	if(t<=1200000){
	    ...
	    if(t<=0){
	    	clearInterval(timer);
	    	var msg2 = "<img src='../img/click-sign.png'/>"+"点我抢券"+"<p>剩余<span>"+$("#remainingNum").val()+"</span>张</p>"
